...
# this endpoint says meta that this webhook is mine and it is valid
@app.get(
    "/",
...
async def whatsapp_webhook(body: WhatsAppWebhookRequest, background_tasks: BackgroundTasks):
    ...
    # Use Pydantic v2-compatible serialization for logging
    logger.info(f"Processing WhatsApp webhook: {body.model_dump()}")
    ...
    try:
        ...
        # Some WhatsApp events (e.g. media_update_failed) are not yet supported
        # by the Pipecat WhatsApp client. Treat these as non-fatal and simply
        # log that they were ignored instead of returning an error.
        if "No supported event found in webhook request" in msg:
            logger.debug(f"Ignoring unsupported WhatsApp event: {msg}")
            return {"status": "ignored", "message": "Unsupported WhatsApp event"}

        logger.warning(f"Invalid webhook request format: {msg}")
        raise HTTPException(status_code=400, detail=f"Invalid request: {msg}")
    except Exception as e:
